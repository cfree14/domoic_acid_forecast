...
data <- data_orig %>% 
  ...
  filter(year > 1998) %>% 
  # Same size catg
  mutate(sample_size_catg=cut(n, 
                              right=F,
                              breaks=c(0, 3, 6, Inf), 
                              labels=c("1-2 crabs", "3-5 crabs", "≥6 crabs")))
...
# Plot survey data
g1 <- ggplot(data, aes(x=date, y=lat_dd, fill=sample_size_catg)) +
  # Season shading
  ...
  # Points
  geom_point(alpha=0.7, pch=21, stroke=0.2) +
  # Limits
  ...
  # Labels
  labs(x="Survey date", y="Latitude (°N)", tag="A") +
  # Legends
  scale_fill_ordinal(name="# of samples in survey") +
  # Theme
  theme_bw() + my_theme +
...
# Plot survey data
g2 <- ggplot(data, aes(x=date, y=lat_dd, fill=sample_size_catg)) +
  # Season shading
  geom_rect(data=seasons_wa, inherit.aes=F, mapping=aes(xmin=open, xmax=close), ymin=46.25, ymax=48.48, fill="grey90") +
  geom_rect(data=seasons_or, inherit.aes=F, mapping=aes(xmin=open, xmax=close), ymin=42, ymax=46.25, fill="grey90") +
  geom_rect(data=seasons_ca_n, inherit.aes=F, mapping=aes(xmin=open, xmax=close), ymin=son_mend_county, ymax=42, fill="grey90") +
  ...
  # Limits
  scale_y_continuous(limits=c(35, 48.5), breaks=seq(34, 48, 2)) +
  scale_x_date(breaks=seq(date_min_do2, date_max_do, by="1 year"), 
               lim=c(date_min_do2, date_max_do),
               labels=year(date_min_do2):year(date_max_do)) +
  # Labels
...
# Plot data
g3 <- ggplot(data_orig, aes(x=n)) +
  geom_histogram(binwidth=1,  fill="grey60") +
  # Reference line
  geom_vline(xintercept=2.5, linetype="dotted") +
  # Labels
  labs(x="# of samples in a survey", y="Number of surveys", tag="C") +
...
layout_matrix <- matrix(c(1,1,
                          2,3), byrow=T, ncol=2)
g <- gridExtra::grid.arrange(g1, g2, g3, layout_matrix=layout_matrix, widths=c(0.7, 0.3))


# Export figure
ggsave(g, filename=file.path(plotdir, "FigS2_survey_sample_size.png"), 
